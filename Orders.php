...
class Orders extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->helper(['url', 'language', 'timezone_helper','order_cancel_email_helper','order_shipped_email_helper','order_processed_email_helper','order_delivered_email_helper','order_return_email_helper','order_assigned_email_helper']);
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (has_permissions('update', 'orders')) {
                if (!empty($_POST['deliver_by'])) {
                        // Validation for 'deliver_by' when it's provided
                        $this->form_validation->set_rules(
                            'deliver_by',
                            'Delivery Boy',
                            'trim|numeric|xss_clean',
                            array('numeric' => 'The Delivery Boy ID must be a numeric value.')
                        );
                    }

                if (!empty($_POST['status'])) {
                    // Validation for 'status' when it's provided
                    $this->form_validation->set_rules(
                        'status',
                        'Status',
                        'trim|xss_clean|in_list[received,processed,shipped,delivered,cancelled,returned]',
                        array('in_list' => 'Please select a valid status.')
                    );
                }
                ...
                if (empty($_POST['order_item_id']) || !isset($_POST['order_item_id'])) {
                    $this->form_validation->set_rules('order_item_id', 'Order Item', 'trim|required|xss_clean', array('required' => "Please select atleast one order product."));
                }
                if (isset($_POST['deliver_by']) && !empty($_POST['deliver_by']) && $_POST['deliver_by'] != '') {
                    $this->form_validation->set_rules('deliver_by', 'Delivery Boy Id', 'trim|numeric|xss_clean');
                }
                ...
                foreach ($_POST['seller_id'] as $sellerId) {
                    $current_status = fetch_details('order_items', ['seller_id' => $sellerId, 'order_id' => $_POST['order_id']], 'active_status,delivery_boy_id');
                ...
                if (!empty($delivery_boy_id)) {
                    if ($awaitingPresent) {
                        ...
                        if (empty($delivery_boy)) {
                            ...
                            if(empty($_POST['status']) && !empty($delivery_boy_id)) {
                                if (isset($current_delivery_boys[0]['delivery_boy_id']) && !empty($current_delivery_boys[0]['delivery_boy_id'])) {
                                $this->response['error'] = true;
                                $this->response['message'] = "Delivery Boy already assigned";
                                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                $this->response['data'] = array();
                                print_r(json_encode($this->response));
                            ...
                            if (isset($user_res[0]) && !empty($user_res[0])) {
                                ...
                                if ($_POST['status'] == 'received') {
                                    ...
                                    if($order_items[0]['active_status'] == 'returned') {
                                        $type = ['type' => "customer_order_returned"];
                                    } else if($order_items[0]['active_status'] == 'cancelled') {
                                        $type = ['type' => "customer_order_cancelled"];
                                    } else if($order_items[0]['active_status'] == 'delivered') {
                                        $type = ['type' => "customer_order_delivered"];
                                    } else if($order_items[0]['active_status'] == 'shipped') {
                                        $type = ['type' => "customer_order_shipped"];
                                    } else if($order_items[0]['active_status'] == 'processed') {
                                        $type = ['type' => "customer_order_processed"];
                                    } else if($order_items[0]['active_status'] == 'received') {
                                        $type = ['type' => "customer_order_received"];
                                    } else {
                                        $type = ['type' => ""];
                                    }
                            ...
                            if ($this->Order_model->update_order(['delivery_boy_id' => $delivery_boy_id], $order_itam_ids, false, 'order_items')) {
                                $delivery_error = false;
                                send_order_assigned_email($_POST['order_id'], $delivery_boy_id);
                            }
                        }
                    }
                }
                ...
                if (!empty($order_items)) {
                    ...
                    if (empty($status)) {
                        $this->response['error'] = true;
                        $this->response['message'] = 'Status is required.';
                        print_r(json_encode($this->response));
                        return false;
                    }
                    ...
                    if ($status) {
                        switch ($status) {
                            case 'processed':
                            send_order_processed_email($order_id);
                                break;
                            ...
                            case 'cancelled':
                                send_order_cancel_email($order_id);
                                break;
                            ...
                            case 'return':
                                send_order_return_emails($order_id, $type);
                                break;
                        ...
                        $this->response['error'] = false;
                        $this->response['message'] = "Order Status Updated Successfully.";
                        print_r(json_encode($this->response));
                        return false;
                        ...
                        $this->response['error'] = true;
                        $this->response['message'] = "Failed to update order status.";
                        print_r(json_encode($this->response));
                    }
                        ...
                        if(isset($_POST['status']) && !empty($_POST['status'])) {
                            ...
                            $this->response['message'] = 'Delivery Boy Updated Successfully';
                        }
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                        $this->response['data'] = array();
    ...
    public function update_order_mail_status()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $order_item_id = $_POST['order_item_id'];
            $status = $_POST['status'];
