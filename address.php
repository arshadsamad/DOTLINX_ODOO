        ...
        <div class="container py-3 py-md-5">
            <nav class="d-inline-block" aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 bg-transparent">
                    <li class="breadcrumb-item"><a href="<?= base_url() ?>" class="text-decoration-none"><?= !empty($this->lang->line('market')) ? $this->lang->line('market') : 'Market' ?></a></li>
                    ...
                    <div class="card">
                        <form action="<?= base_url('my-account/add-address') ?>" method="POST" id="add-address-form" class="mt-4 p-4">
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12 col-xs-12 form-group state">
                                    <label for="state_id" class="control-label"><?= !empty($this->lang->line('state')) ? $this->lang->line('state') : 'State' ?></label>
            ...
            <div class="modal-body ps-10 pt-0">
                <form action="<?= base_url('my-account/edit-address') ?>" method="POST" id="edit-address-form" class="mt-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-sm-12 col-xs-12 form-group edit-state">
                            <label for="edit_state" class="control-label"><?= !empty($this->lang->line('state')) ? $this->lang->line('state') : 'State' ?></label>
                            <select class="form-control" name="state_id" id="edit_state">
                                <option value=""><?= !empty($this->lang->line('select_state')) ? $this->lang->line('select_state') : '--Select State--' ?></option>
                                <?php foreach ($states as $row) { ?>
                                <option value="<?= $row['id'] ?>"><?= $row['name'] ?></option>
                                <?php } ?>
...
window.editAddress = {
    'click .edit-address': function(e, value, row, index) {
        ...
        if (row.city_id == 0 || row.city_id == "") {
            ...
            $("#edit_state").val(row.state_id).trigger('change');
            // $('.edit_pincode').addClass('d-none');
            $('.other_pincode').removeClass('d-none');
            $("#other_pincode_value").val(row.pincode);
        } else {
