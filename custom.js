...
var search_products = $(".search_product").select2({
    ajax: {
        ...
        delay: 250,
        data: function (e) {
            var seller_slug = $(".search_product").data("seller");
            return {
                search: e.term,
                page: e.page,
                seller: seller_slug
            ...
            $("#quick-view").iziModal({
                ...
                onOpening: function (modal) {
                    ...
                    $.getJSON(base_url + 'products/get-details/' + modal.$element.data('dataProductId'), function (data) {
                        ...
                        console.log(data);
                        console.log(data.check_deliverability.city_wise_deliverability);
                        // console.log(statistics);
                        var stockAvailabile = true;
                        if(parseInt(data.availability) == 0) {
                            stockAvailabile = false;
                        }
                        var total_images = 0;
                        ...
                        var main_images = '<div class="swiper-slide swiper-image"><div class=product-view-image-container">';
                            if(stockAvailabile) {
                                main_images += '<img src="' + data.image_md + '" class="rounded" alt="" />';
                            } else {
                                main_images += '<div class="no-stock-container">';
                                    main_images += '<img src="' + data.image_md + '" class="rounded" alt="" style="object-fit: unset !important;"/>';
                                    main_images += '<div class="no-stock-txt-container">';
                                        main_images += '<div class="no-stock-txt-banner">';
                                            main_images += '<span class="no-stock-text">OUT OF STOCK</span>';
                                        main_images += '</div">';
                                    main_images += '</div">';
                                main_images += '</div">';
                            }
                            main_images += '</div></div>';
                        var mainImage = $(main_images);
                        $(".swiper-wrapper-main").append(mainImage);

                        var imgMob = '<img src="' + data.image_md + '">';
                        if(!stockAvailabile) {
                            imgMob = '<div class="no-stock-container">';
                                imgMob += '<img src="' + data.image_md + '" alt="" style="object-fit: unset !important;"/>';
                                imgMob += '<div class="no-stock-txt-container">';
                                    imgMob += '<div class="no-stock-txt-banner">';
                                        imgMob += '<span class="no-stock-text">OUT OF STOCK</span>';
                                    imgMob += '</div">';
                                imgMob += '</div">';
                            imgMob += '</div">';
                        }
                        var mobile_slider_image = $('<div class="swiper-slide text-center">'+imgMob+'</div>');
                        ...
                        if(stockAvailabile) {
                            ...
                            if (thumb_images && thumb_images.length > 1) {
                        ...
                        if (data.check_deliverability.city_wise_deliverability == 1) {
                            var stateOptStr = '<option value="">--Select State --</option>';
                            $.each(data.states, function (i, state) {
                                stateOptStr += '<option value="'+state["id"]+'">'+state["text"]+'</option>';
                            });
                            if (stockAvailabile && data.type != "digital_product") {
                                variant_attributes +=
                                    '<form class="mt-2 validate_state_availability "   method="post" >' +
                                    ...
                                    '<select id="check_state" class="form-control" name="state">' +
                                       stateOptStr +
                                    '</select>' +
                                    '</div>&nbsp;&nbsp;' +
                                    '<button type="submit" class="btn btn-sm ml-0 btn-primary check-availability" data-product_id="' + data.id + '"  data-city=""  id="validate_state_availability">Check Availability</button>' +
                                    '</div>' +
                        ...
                        if(!stockAvailabile) {
                            $('#modal-add-to-cart-button').remove();
                            $('#modal-buy-now-button').remove();
                            $('#qty_panel').remove();
                ...
                if (1 == is_loggedin) $.ajax({
                    ...
                    success: function (e) {
                        if (csrfName = e.csrfName, csrfHash = e.csrfHash, 0 == e.error) {
                            ...
                            if (window.location.href === 'https://movingbazaar.com/cart') {
...
$("#back_to_top").on("click", function () {
    ...
    $("#seller_search_btn").on("click", function (e) {
        e.preventDefault();
        var t = $("#seller_search_txt").val();
        location.href = setUrlParameter(location.href, "seller_search", t)
    }),
    $("#seller_clear_btn").on("click", function (e) {
        e.preventDefault();
        location.href = setUrlParameter(location.href, "seller_search", '');
    }),
...
function customer_wallet_query_paramss(e) {
                        ...
                        $.each(e.data.items, function (e, a) {
                            ...
                            var r = void 0 !== a.product_variants.variant_values && null != a.product_variants.variant_values ? a.product_variants.variant_values : "",
                                s = a.special_price < a.price && 0 != a.special_price ? a.special_price : a.price;
                                                         t += '<div class="shopping-cart"><div class="shopping-cart-item d-flex justify-content-between mb-4" title = " ' + a.name + '"><div class="d-flex flex-row gap-3"><figure class="rounded cart-img"><a href="' + base_url + 'products/details/' + a.product_slug + '"><img src="' + base_url + a.image + '" alt="'+a.name+'" style="object-fit: contain;"></a></figure><div class="w-100"><a href="' + base_url + 'products/details/' + a.slug + '"><h3 class="post-title fs-16 lh-xs mb-1"  title = " ' + a.name + '">' + a.name + "</h3></a><span>" + r + '</span><p class="price"><ins><span class="amount">' + currency + " " + s + '</span></ins></p><div class="product-pricing d-flex py-2 px-1 w-100"><div class="align-items-center d-flex p-2 w-15"><input type="number" name="header_qty" class="form-control d-flex align-items-center" value="' + a.qty + '" data-id="' + a.product_variant_id + '" data-price="' + a.price + '" min="' + n + '" max="' + c + '" step="' + l + '" ></div><div class="product-line-price align-self-center px-1">' + currency + (a.qty * s) + '</div></div></div></div><div class="product-sm-removal"><button class="remove-product btn btn-sm btn-danger rounded-1 p-1 py-0" data-id="' + a.product_variant_id + '"><i class="uil uil-trash-alt"></i></button></div></div></div>'                        }),
    ...
    $('#state_id').on('change', function (e) {
        ...
        $.ajax({
            type: 'GET',
            url: base_url + 'my-account/get_parent_sub_states?parent_state_id='+stateId,
            dataType: 'json',
            success: function (result) {
                if (result.data.length > 0) {
                    var html = '';
                    html += '<option value="">--Select Sub State--</option>';
                    $.each(result.data, function (i, e) {
                        html += '<option value=' + e.id + '>' + e.name + '</option>';
                    });
                    $('#sub_state_id').html(html);
                } else {
                    $('#sub_state_id').empty();
                }
            }
    ...
    $("#sub_state").select2({
        ajax: {
            url: base_url + 'my-account/get_sub_states',
            type: "GET",
            dataType: 'json',
            delay: 250,
            data: function (params) {
        ...
        minimumInputLength: 1,
        theme: 'bootstrap4',
        placeholder: 'Search for sub state',
        containerCssClass: 'city-container',
        dropdownCssClass: 'city-dropdown',
        dropdownParent: $("#add-address-form"),
    ...
    e = null !== localStorage.getItem("compare") ? e : null, $.ajax({
        ...
        success: function (t) {
            ...
            0 == t.error ? (null !== e && a > 0 && (r += '<div class="align-self-end mb-7"><div class="compare-removal"><button class="remove-compare btn btn-danger btn-sm" >Clear Compare</button></div></div></div><div><table class="compare-table table-bordered"><tbody><tr><th class="compare-field w-19"> </th>', $.each(t.data.product, function (e, t) {
                ...
                if(parseInt(t.availability) != 0) {
                    availability = true;
                }
                var imgTag = '<a href="products/details/' + t.slug + '"><img class="pro-img" src="' + t.image + '" style="object-fit:cover;"></a>';
                var pTitle = '<a class="text-decoration-none" href="products/details/' + t.slug + '">' + shortDescriptionWordLimit(t.name) + '</a>';
                if(!availability) {
                    var pImg = '<a><img class="pro-img" src="' + t.image + '" style="object-fit: unset !important;"></a>'
                    imgTag = '<div class="no-stock-container">';
                        imgTag += '<img src="' + pImg + '" alt="" style="object-fit: unset !important;"/>';
                        imgTag += '<div class="no-stock-txt-container">';
                            imgTag += '<div class="no-stock-txt-banner">';
                                imgTag += '<span class="no-stock-text">OUT OF STOCK</span>';
                            imgTag += '</div">';
                        imgTag += '</div">';
                    imgTag += '</div">';
                    pTitle = '<a class="text-decoration-none" href="#">' + shortDescriptionWordLimit(t.name) + '</a>';
                }
                ...
                if (r += '<td class="compare_item text-center text-justify">'+
                    '<div class="p-5"><div class="text-right"><a class="remove-compare-item"data-product-id="' + t.id + '" '+
...
$('#validate-state-form').on('submit', function (e) {
        e.preventDefault()
        var formdata = new FormData(this)
        formdata.append(csrfName, csrfHash)
        $.ajax({
            type: 'POST',
            url: base_url + 'products/check_state',
            data: formdata,
            beforeSend: function () {
                $("#validate_state").html("Please Wait..").attr("disabled", !0)

                // $('#validate_state').html('<div class="spinner-border" role="status">' +
            ...
            success: function (result) {
                csrfHash = result.csrfHash
                console.log(result);
                $('#validate_state').html('Check Availability').attr('disabled', false)
                if (result.error == false) {
                    $('#add_cart').removeAttr('disabled')
                    $('.buy_now').removeAttr('disabled')
    ...
    $('#validate-state-availability-form').on('submit', function (e) {
        e.preventDefault()
        var formdata = new FormData(this)
        formdata.append(csrfName, csrfHash)
        $.ajax({
            type: 'POST',
            url: base_url + 'products/check_state_availability',
            data: formdata,
            beforeSend: function () {
                $("#validate_state").html("Please Wait..").attr("disabled", !0)
            },
...
$(document).on("submit", ".validate_zipcode_quick_view", function (e) {
    ...
    $(document).on("submit", ".validate_state_availability", function (e) {
        e.preventDefault();
        var t = new FormData(this);
        t.append(csrfName, csrfHash), $.ajax({
            type: "post",
            ...
            data: t,
            beforeSend: function () {
                $("#validate_state_availability").html("Please Wait..").attr("disabled", !0)
            },
            cache: !1,
            contentType: !1,
            processData: !1,
            ...
            success: function (e) {
                csrfHash = e.csrfHash,
                    $("#validate_state_availability").html("Check Availability").attr("disabled", !1)
                if (e.error == false) {
                    $('#modal-add-to-cart-button').removeAttr('disabled')
                    $('modal-buy-now-button').removeAttr('disabled')
                    $('#error_box1').html(e.message)
...
swiperContainers.forEach((container) => {
    const swiperInstance = new Swiper(container, {
        ...
        slidesPerView: "auto",
        pagination: {
            el: container.querySelector(".slide-swiper-pagination"),
            clickable: true
        },
    });

    let activeIndex = 0;
    const slides = container.querySelectorAll(".swiper-slide-category");

    slides.forEach((slide, index) => {
        if (slide.classList.contains("border-grey")) {
            activeIndex = index;
        }
    });
