...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        for order in self:
            ...
            if order.amount_total < order.partner_id.min_credit_limit:
                raise UserError(
                            _('The sale amount is %s, and the minimum credit limit is %s. Please check your credit limit' % (order.amount_total, order.partner_id.min_credit_limit)))

            if order.amount_total > order.partner_id.remaining_credit_limit and order.partner_id.remaining_credit_limit:
                if not order.payment_term_id.name == 'Immediate Payment':
                    raise UserError(
                                _('The sale amount of %s has exceeded the credit limit of %s Please check your credit limit, which is currently set at %s' % (order.amount_total, order.partner_id.remaining_credit_limit, order.partner_id.credit_limit)))
            ...
            if order.order_line and order.state in ['draft', 'credit_hold'] and order.partner_id.credit_limit > 0:
                ...
                if not check_false[0] >= check_false[1] and order.amount_total > 0.0:
                    if not order.payment_term_id.name == 'Immediate Payment':
                        raise UserError(
                            _('The sale amount of %s has exceeded the credit limit of %s due to previous sales totaling %s. '
                              '\n Otherwise, You can confirm this Sale Order only when its Payment term is Immediate!' %(order.amount_total,check_false[0] ,check_false[2])))
    ...
    def credit_approve(self):
        if not self.env.user.has_group('sales_team.group_sale_manager') and not self.env.user.has_group('account.group_account_manager'):
            ...
            view_id = self.env.ref('sale_credit_limit.view_approval_credit_limit', False)
    ...
    def cancel_order_on_cc(self):
        if not self.env.user.has_group('sales_team.group_sale_manager') and not self.env.user.has_group('account.group_account_manager'):
            ...
            view_id = self.env.ref('sale_credit_limit.view_cancel_credit_limit', False)
            return {'type': 'ir.actions.act_window',
