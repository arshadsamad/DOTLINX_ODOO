...
$hide_product_nav = isset($hide_product_nav) ? $hide_product_nav : false;
// echo "<pre>";
...
<!-- header starts -->
<div class="fixed-bottom d-xs-block d-sm-block d-lg-none mobile-toolbar-container">
    <ul class="nav justify-content-center">
        <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
        <li class="nav-item text-center">
            <a class="nav-link py-2" href="<?= base_url() . 'sellers/' . $seller_slug ?>">
                <i class="uil h4 uil-store"></i><br>Shop</a>
        </li>
        <?php } ?>
        <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
        <li class="nav-item text-center">
            <a class="nav-link py-2" href="#" data-bs-toggle="offcanvas" data-bs-target="#offcanvas-search">
                <i class="uil h4 uil-search"></i><br>Search</a>
        </li>
        <?php } ?>
        <li class="nav-item text-center">
            <!-- check login -->
            <?php if ($this->ion_auth->logged_in()) { ?>
            <a href="<?= base_url('my-account/favorites') ?>" target="_blank" class="nav-link py-2" aria-label="favorites">
                <i class="uil h4 uil-heart h4"></i><br>Wishlist
            </a>
            <?php } else { ?>
            <a href="#" class="nav-link py-2" aria-label="favorites" data-bs-toggle="modal" data-bs-target="#modal-signin">
                <i class="uil h4 uil-heart"></i><br>Wishlist
            </a>
            <?php } ?>
        </li>
        <?php if ($this->ion_auth->logged_in()) { ?>
        <li class="nav-item text-center">
            <a href="<?= base_url('login/logout') ?>" class="nav-link py-2" aria-label="logout"><i class="uil uil-signout h4"></i><br>
                <?= !empty($this->lang->line('logout')) ? $this->lang->line('logout') : 'Logout' ?>
            </a>
        </li>
        <!-- <li class="nav-item text-center dropdown active">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-lg">
                                        <a href="<?= base_url('my-account') ?>" target="_blank"
                                        ...
                                        <a href="<?= base_url('my-account/orders') ?>" target="_blank"
                                        ...
                                        <a href="<?= base_url('login/logout') ?>" class="dropdown-item text-decoration-none"
                                            aria-label="logout"><i
                                                class="link-color mr-2 text-primary uil uil-signout"></i><?= !empty($this->lang->line('logout')) ? $this->lang->line('logout') : 'Logout' ?></a>
                                    </div>
                                </li> -->

        <?php } else { ?>
        <li class="nav-item text-center">
            <a href="#" aria-label="login" class="nav-link py-2" data-bs-toggle="modal" data-bs-target="#modal-signin"><i class="uil uil-user-circle h4"></i><br><?= !empty($this->lang->line('login')) ? $this->lang->line('login') : 'Login' ?>
            </a>
        </li>
...
<header class="wrapper bg-soft-primary">
    ...
    <div class="content-wrapper sticky-content">
        <header class="wrapper bg-light">
                    ...
                    <div class="d-flex flex-fill">
                        <ul class="align-items-center flex-row ms-auto navbar-nav">
                            ...
                            <li class="nav-item dropdown active">
                                ...
                                <div class="dropdown-menu dropdown-menu-lg">
                                    <!-- <a href="<?= base_url('my-account/wallet') ?>" target="_blank" class="dropdown-item text-decoration-none" aria-label="wallet"><i class="uil uil-wallet mr-2 text-primary link-color"></i> <?= $settings['currency'] ?><?= ' ' . isset($user->balance) && !empty($user->balance) ? number_format($user->balance, 2) : 0.0 ?></a> -->
                                    <a href="<?= base_url('my-account') ?>" target="_blank" class="dropdown-item text-decoration-none" aria-label="my profile"><i class="uil uil-user text-primary link-color mr-2"></i>
                                        <?= !empty($this->lang->line('profile')) ? $this->lang->line('profile') : 'Profile' ?>
                                    </a>
                                    <a href="<?= base_url('my-account/orders') ?>" target="_blank" class="dropdown-item text-decoration-none" aria-label="orders"><i class="link-color mr-2 text-primary uil uil-history"></i>
                                        <?= !empty($this->lang->line('orders')) ? $this->lang->line('orders') : 'Orders' ?>
                                    </a>
            ...
            <?php if (isset($limit_navbar) && $limit_navbar == true) { ?>
            ...
            <nav class="navbar navbar-expand-lg center-nav transparent navbar-light">
                <div class="container flex-lg-row flex-nowrap align-items-center">
                    <span class="nav-item px-2 d-lg-none">
                        <button class="btn btn-link btn-sm fs-20 text-body mr-2 offcanvas-nav-btn p-0 text-decoration-none uil uil-bars"><span></span></button>
                    </span>
                    <div class="navbar-brand w-100 py-1 py-lg-0 logo-img">
                        <a class="ml-2" href="<?= base_url() ?>">
                            <?php if (ALLOW_MODIFICATION == 0) { ?>
                            <img src="<?= base_url("assets/front_end/modern/img/logo/orange.png") ?>" alt="site-logo image" class="brand-logo-link logo-img" style="object-fit: contain;">
                            <?php } else { ?>
                    ...
                    <div class="navbar-collapse offcanvas offcanvas-nav offcanvas-start">
                        ...
                        <?php if ($hide_product_nav) { ?>
                        <div class="offcanvas-body ms-lg-auto d-flex flex-column h-100">
                            <ul class="navbar-nav">
                                <li class="nav-item">
                                    <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
                                    <a class="nav-link <?= ($current_url == base_url('')) ? 'active' : '' ?>" aria-current="page" aria-label="home" href="<?= base_url() . 'sellers/' . $seller_slug ?>"><?= !empty($this->lang->line('seller_home')) ? $this->lang->line('seller_home') : 'Seller Home' ?></a>
                                    <?php } else { ?>
                                    <a class="nav-link <?= ($current_url == base_url('')) ? 'active' : '' ?>" aria-current="page" aria-label="home" href="<?= base_url() ?>"><?= !empty($this->lang->line('market')) ? $this->lang->line('market') : 'Market' ?></a>
                                    <?php } ?>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link <?= ($current_url == base_url('home/contact-us')) ? 'active' : '' ?>" href="<?= base_url('home/contact-us') ?>" target="_blank" aria-label="contact-us"><?= !empty($this->lang->line('contact_us')) ? $this->lang->line('contact_us') : 'Contact Us' ?></a>
                                </li>
                            </ul>
                            <!-- /.offcanvas-footer -->
                        </div>
                        ...
                        <div class="offcanvas-body ms-lg-auto d-flex flex-column h-100">
                            <ul class="navbar-nav">
                                ...
                                <li class="nav-item">
                                    <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
                                    <a class="nav-link <?= ($current_url == base_url('products')) ? 'active' : '' ?>" href="<?= base_url() . 'sellers/' . $seller_slug . '/products' ?>" aria-label="products"><?= !empty($this->lang->line('products')) ? $this->lang->line('products') : 'Products' ?></a>
                                    <?php } else { ?>
                                ...
                                <!-- <li class="nav-item">
                                    <a class="nav-link <?= ($current_url == base_url('home/faq')) ? 'active' : '' ?>" href="<?= base_url('home/faq') ?>" target="_blank" aria-label="faq"><?= !empty($this->lang->line('faq')) ? $this->lang->line('faq') : 'FAQs' ?></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link <?= ($current_url == base_url('blogs/')) ? 'active' : '' ?>" href="<?= base_url('blogs/') ?>" target="_blank"><?= !empty($this->lang->line('blogs')) ? $this->lang->line('blogs') : 'Blogs' ?></a>
                                </li> -->
                            </ul>
                            <!-- /.offcanvas-footer -->
                        </div>
                    ...
                    <div class="navbar-other w-100 d-flex ms-auto position_rtl">
                        <ul class="navbar-nav flex-row align-items-center ms-auto ul_rtl">
                            ...
                            <li class="nav-item d-none d-lg-block">
                                <a class="nav-link desktop-search" data-bs-toggle="offcanvas" data-bs-target="#offcanvas-search"><i class="uil uil-search"></i></a>
                            </li>
                            <?php } ?>
                            <li class="nav-item d-none d-lg-block">
                                <?php if ($this->ion_auth->logged_in()) { ?>
                            ...
                            <li class="nav-item d-flex flex-row align-items-center">
                                <?php if ($this->ion_auth->logged_in()) { ?>
                                <a href="<?= base_url('compare') ?>" target="_blank" class="nav-link d-flex" onclick=display_compare() data-product-id="<?= ($row['id']) != 0 ? $row['id'] : '' ?>" aria-label="compare">
                                    <i class="uil uil-exchange-alt"></i>
                                    <span class="badge bg-primary badge-cart badge-sm" id='compare_count'></span>
                                </a>
                                <?php } else { ?>
                                <a href="#" class="nav-link d-flex" aria-label="compare" data-bs-toggle="modal" data-bs-target="#modal-signin">
                                    <i class="uil uil-exchange-alt"></i>
                                </a>
                                <?php } ?>
                            </li>
            ...
            <?php } ?>
            <!-- /.navbar -->
            <div class="container d-none pb-1 mobile-searchbar">
                <div class="w-100">
            ...
            <div class="offcanvas offcanvas-end bg-light" id="offcanvas-cart" data-bs-scroll="true" aria-modal="true" role="dialog">
                ...
                <div class="offcanvas-header">
                    <h3 class="mb-0">
                        <?= !empty($this->lang->line('shopping_cart')) ? $this->lang->line('shopping_cart') : 'Shopping Cart' ?>
                    </h3>
                ...
                <div class="offcanvas-footer flex-column text-center container" style="margin-bottom:35px;">
                    <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
                    <a class="btn btn-red rounded-pill ms-6 mr-4 mb-5" href="<?= base_url() . 'sellers/' . $seller_slug ?>">
                        <?= !empty($this->lang->line('return_to_shop')) ? $this->lang->line('return_to_shop') : 'Return to Shop' ?>
                    </a>
                    <?php } ?>
                    <?php if ((isset($user->id)) == 1) { ?>
                    <?php if (isset($seller_slug) && !empty($seller_slug) && $seller_slug != '') { ?>
                    <a href="<?= base_url() . 'sellers/' . $seller_slug . '/cart' ?>" class="btn btn-primary btn-icon btn-icon-start rounded-pill mb-4 view_cart_button ms-6 mr-4">
            ...
            <div class="offcanvas offcanvas-top bg-light" id="offcanvas-search" data-bs-scroll="true">
                <div class="container d-flex flex-row py-4">
                    <!-- <form class="w-100"> -->
                    <select class="form-control rounded-0 search_product" type="text" aria-label="Search" data-seller="<?= $seller_slug ?>"></select>
                    <!-- </form> -->
                    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
            </div>
